...
def get_author_name(scopus_id: str, headers: dict) -> str:
    """Author Retrieval APIで研究者名を取得する。失敗時は空文字を返す。"""
    try:
        resp = requests.get(
            f"{AUTHOR_URL}/{scopus_id}",
            headers=headers,
            timeout=30,
        )
        resp.raise_for_status()
        entry = resp.json()["author-retrieval-response"][0]
        preferred = entry["author-profile"]["preferred-name"]
        return f"{preferred['given-name']} {preferred['surname']}"
    except (requests.RequestException, KeyError, IndexError) as e:
        st.warning(f"著者名取得エラー (ID={scopus_id}): {e}")
        return ""
...
if st.button("論文情報を取得", type="primary"):  # テーマカラー（オレンジ）
    ...
    insttoken = st.secrets.get("SCOPUS_INSTTOKEN", "")
    if insttoken:
        headers["X-ELS-Insttoken"] = insttoken
    citescore_cache: dict = {}
    author_name_cache: dict = {}
    ...
    for idx, researcher_id in enumerate(researcher_ids):
        ...
        # 著者名を取得
        author_name = get_author_name(researcher_id, headers)
        author_name_cache[researcher_id] = author_name
        ...
        for pub in publications:
            ...
            pub["author_name"] = author_name_cache.get(researcher_id, "")
    ...
    st.dataframe(
        ...
        column_config={
            "研究者ID": st.column_config.LinkColumn("研究者ID", display_text=r"authorId=(\d+)"),
            "DOIリンク": st.column_config.LinkColumn("DOIリンク", display_text=r"doi\.org/(.+)"),
            "Scopusページ": st.column_config.LinkColumn("Scopusページ", display_text=r"publications/(\d+)"),
            "ジャーナルページ": st.column_config.LinkColumn("ジャーナルページ", display_text=r"sourceid/(\d+)"),
        },
    )

    # --- CSVダウンロード ---
    fieldnames = [
        "研究者名", "研究者ID", "論文タイトル", "掲載誌", "発表年", "パーセンタイル",

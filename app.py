...
def get_researcher_publications(
    ...
    while start < max_results:
        ...
        for entry in entries:
            ...
            authors = ", ".join(
                a.get("authname", "") for a in entry.get("author", []) if a.get("authname")
            )
            results.append({
...
if st.button("論文情報を取得", type="primary"):  # テーマカラー（オレンジ）
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "著者": st.column_config.TextColumn("著者", width=160),
    ...
    def download_csv():
        fieldnames = [
            "Scopus Author ID", "h-index", "論文タイトル", "著者", "掲載誌", "発行年", "パーセンタイル（発行年）",
            "被引用数", "CiteScore（発行年）",
